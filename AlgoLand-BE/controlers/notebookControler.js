...
async function getNotebook(username,code){
    let retrievedNotebooks;
    retrievedNotebooks = notebookMdl.find({owner: username,code})
        .then((dataRes)=>{
            if(dataRes != null)
                {return dataRes}
        })
...
function retrieveNotebook(req, res){
    let { username,title, singular,code} = req.query;
    singular = eval(singular);

    if(!singular)
        getNotebook(username,code).then(notebook =>{
